...
int partition (int arr[],int si, int ei){
    int pivotElement = arr[(si+ei)/2];//(si+ei)/2 as we are ataking randomized pivot Element
    int count = 0;
    for(int i=si;i<=ei;i++){
    ...
    swap(arr[(si+ei)/2],arr[pivotIdx]);//pivot eleemnt apne sahi index mein chala gya
